...
class Category extends Model{
	...
	public function getProductsPage($page = 1, $itensPerPage = 3){
		...
		$sql = new Sql();
		$results = $sql->select("SELECT SQL_CALC_FOUND_ROWS * FROM tb_products a INNER JOIN 
			tb_productscategories b ON a.idproduct = b.idproduct INNER JOIN tb_categories c 
			ON c.idcategory = b.idcategory WHERE c.idcategory = :idcategory
	...
	public static function getPage($page = 1, $itensPerPage = 5){
		$start = ($page - 1) * $itensPerPage;
		$sql = new Sql();
		$results = $sql->select("SELECT SQL_CALC_FOUND_ROWS * FROM tb_categories ORDER BY descategory LIMIT $start, $itensPerPage"
			);
		$resultsTotal = $sql->select("SELECT FOUND_ROWS() AS nrtotal;");
		...
		$start = ($page - 1) * $itensPerPage;
		$sql = new Sql();
		$results = $sql->select("SELECT SQL_CALC_FOUND_ROWS * FROM tb_categories WHERE descategory LIKE :search ORDER BY descategory
			LIMIT $start, $itensPerPage",array(
				':search'=>'%'.$search.'%'
			));
		$resultsTotal = $sql->select("SELECT FOUND_ROWS() AS nrtotal;");
