...
use \TH\model\Cart;
	...
	class User extends Model{
		...
		public static function login($login,$password){
			...
			$data = $results[0];
				...
				if(password_verify($password, $data["despassword"]) === true){
					...
					$user->setData($data);		//$user->setiduser($data["iduser"]); //Ao invés de passar um campo, iremos passar o array inteiro, por isso esse metodo substituido 
		...
		public function getOrders(){
			...
			ON f.idperson = d.idperson WHERE a.iduser = :iduser", array(
				':iduser'=>$this->getiduser()));
		...
		$sql = new Sql();
		$results = $sql->select("SELECT SQL_CALC_FOUND_ROWS * FROM tb_users a INNER JOIN tb_persons b USING(idperson) ORDER BY b.desperson LIMIT $start, $itensPerPage"
			);
		...
		$start = ($page - 1) * $itensPerPage;
		$sql = new Sql();
		$results = $sql->select("SELECT SQL_CALC_FOUND_ROWS * FROM tb_users a INNER JOIN tb_persons b USING(idperson) WHERE b.desperson LIKE :search OR b.desemail = :search OR a.deslogin LIKE :search  ORDER BY b.desperson
